/// <summary>
/// Report Sales - Credit Memo Servitec (ID 50001).
/// </summary>
Report 50001 "Sales - Credit Memo Servitec"
{

    Permissions = TableData 7190 = rimd;
    CaptionML = ENU = 'Sales - Invoice Servitec',
               ESP = 'Ventas - Abono Servitec';
    DefaultLayout = RDLC;
    RDLCLayout = './src/report/layout/SalesCRMemo.rdlc';


    dataset
    {
        dataItem("Sales Invoice Header"; 114)
        {
            ...
            column(YourReference_SalesInvoiceHeader;
            "Sales Invoice Header"."Your Reference")
            { }

            column(CorrectedInvoiceNo_SalesInvoiceHeader;
            "Sales Invoice Header"."Corrected Invoice No.")
            { }
            ...
            dataItem(CopyLoop; Integer)
            {
                ...
                dataItem(PageLoop; integer)
                {
                    ...
                    "Sales Invoice Header"."Return Order No.")
                    { }
                    ...
                    column(CorrInvNo_SalesCrMemoHeader;
                    "Sales Invoice Header"."Corrected Invoice No.")
                    { }
                    ...
                    dataItem(DimensionLoop1; integer)
                    {
                        ...
                        column(Number_DimensionLoop1;
                        Number)
                        { }

                        column(HdrDimsCaption;
                        HdrDimsCaptionLbl)
                        { }
                    ...
                    dataItem("Sales Invoice Line"; 115)
                    {
                        ...
                        dataItem("Sales Shipment Buffer"; integer)
                        {
                            ...
                            column(PostingDate_SalesShipmentBuffer;
                            FORMAT(SalesShipmentBuffer."Posting Date"))
                            { }

                            column(Quantity_SalesShipmentBuffer;
                        ...
                        dataItem(AsmLoop; Integer)
                        {
                            ...
                            column(TempPostedAsmLineDescrip;
                    ...
                    dataItem(VatCounterLCY; integer)
                    {
                        ...
                        column(VATAmountLineVAT1;

                        VATAmountLine."VAT %")
                        { DecimalPlaces = 0 : 5; }

                        column(VATAmtLineVATIdentifier1;
                        VATAmountLine."VAT Identifier")
                        { }

                        column(VALVATBaseLCYCaption1;
                        VALVATBaseLCYCaption1Lbl)
                        { }
                        trigger OnPreDataItem()
                        BEGIN
                ...
                BEGIN
                    ...
                    IF NOT CurrReport.PREVIEW THEN
                        SalesCrMemoCountPrinted.RUN("Sales Invoice Header");
                END;
            }
            ...
            BEGIN
                ...
                // }
                FormatAddr.SalesCrMemoShipTo(ShipToAddr, CustAddr, "Sales Invoice Header");
        ...
        layout
        {
            area(Content)
            {
                ...
                Group(Opciones)
                {
                    ...
                    field("Nº copias"; NoOfCopies)
                    {
                        ApplicationArea = All;
                    }


                    field("Mostrar información interna"; ShowInternalInfo)
                    { ApplicationArea = All; }

                    field("Log interacción"; LogInteraction)
                    {
                        Enabled = LogInteractionEnable;
                        ApplicationArea = All;
                    }

                    field("Mostrar componentes del ensamblado"; DisplayAssemblyInformation)
                    {
                        ApplicationArea = All;
                    }
    ...
    VAR
        ...
        Text004: TextConst ENU = 'Credit Memo %1', ESP = 'Factura Rectificativa %1';
        ...
        Text011: TextConst ENU = 'Sales - Prepmt. Credit Memo %1', ESP = 'Ventas - Abono prepago %1';
        ...
        InvoiceNoCaptionLbl: TextConst ENU = 'Invoice No.', ESP = 'N.º factura';
        ...
        NoFacCorrLbl: TextConst ENU = 'Corrected Invoce No.', ESP = 'No. factura corregida';
    ...
    BEGIN
        NextEntryNo := 1;
        IF "Sales Invoice Line"."Return Receipt No." <> '' THEN
            IF ReturnReceiptHeader.GET("Sales Invoice Line"."Return Receipt No.") THEN
                EXIT(ReturnReceiptHeader."Posting Date");
        IF "Sales Invoice Header"."Return Order No." = '' THEN
    ...
    BEGIN
        ...
        IF ReturnReceiptLine.FIND('-') THEN
            REPEAT
                IF "Sales Invoice Header"."Get Return Receipt Used" THEN
    ...
    BEGIN
        IF "Sales Invoice Header"."Prepayment Credit Memo" THEN
            EXIT(Text011);
        EXIT(Text004);
    END;
